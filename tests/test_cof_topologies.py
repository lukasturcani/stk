...
def test_honeycomb(cof_bb1):
    bb2 = stk.StructUnit3(join('data', 'cof', 'aldehyde3f.mol'))
    cof = stk.Periodic([cof_bb1, bb2], stk.Honeycomb())
...
def test_hexagonal(cof_bb1):
    bb2 = stk.StructUnit3(join('data', 'cof', 'aldehyde6f.mol'))
    cof = stk.Periodic([cof_bb1, bb2], stk.Hexagonal())
...
def test_square(cof_bb1):
    bb2 = stk.StructUnit3(join('data', 'cof', 'aldehyde4f_1.mol'))
    cof = stk.Periodic([cof_bb1, bb2], stk.Square())
...
def test_kagome(cof_bb1):
    bb2 = stk.StructUnit3(join('data', 'cof', 'aldehyde4f_2.mol'))
    cof = stk.Periodic([cof_bb1, bb2],
                       stk.Kagome(multitopic_aligners=[1, 0, 1]))
...
def test_boron_cof(cof_bb1):
    cof_bb1 = stk.StructUnit2.smiles_init('Oc1cc2cc(O)c(O)nc2cc1O',
                                          'diol')
...
def test_nolinkerhoneycomb(cof_bb1):
    cof_bb1 = stk.StructUnit3.smiles_init('O=Cc1cc(C=O)nc(C=O)c1',
                                          'aldehyde')
    bb2 = stk.StructUnit3.smiles_init('NCc1cc(CN)nc(CN)n1', 'amine')
    cof = stk.Periodic([cof_bb1, bb2], stk.NoLinkerHoneycomb())
