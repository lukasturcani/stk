...
class EightPlusSixteen(Cage):
    ...
    *Multi-Building Block Cage Construction*, a
    ...
    _x = 2
    _vertex_prototypes = (
        NonLinearCageVertex(0, [-0.5*_x, 0.5*_x, -0.35*_x]),
        NonLinearCageVertex(1, [-0.5*_x, -0.5*_x, -0.35*_x]),
        NonLinearCageVertex(2, [0.5*_x, -0.5*_x, -0.35*_x]),
        NonLinearCageVertex(3, [0.5*_x, 0.5*_x, -0.35*_x]),

        NonLinearCageVertex(4, [-_x*np.sqrt(2)/2, 0, _x*0.35]),
        NonLinearCageVertex(5, [0, -_x*np.sqrt(2)/2, _x*0.35]),
        NonLinearCageVertex(6, [_x*np.sqrt(2)/2, 0, _x*0.35]),
        NonLinearCageVertex(7, [0, _x*np.sqrt(2)/2, _x*0.35]),
    )
