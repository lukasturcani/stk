...
class TopologyGraph(typing.Generic[_V]):
    ...
    Notes:
        ...
        *Adding New Topology Graphs*
        ...
        implementation of :class:`.TopologyGraph`, you mostly just need
        to implement a :class:`.Vertex` subclass, which is much easier
        than figuring out the whole construction process from scratch.
        In addition, you get benefits like parallel construction for
        free, as it is included in the default implementation.
        ...
        add additional hooks during construction, by extending the
        methods of this class. If you do this, make sure to override
        :meth:`._get_construction_state` to return your subclass of
        :class:`.ConstructionState`, rather than the base class, as is
        done by default. You can subclass and extend the methods of any
        class as you wish, but it would be unusual if this doesn't
        ...
        *The Default Implementation*
        ...
        normally add the bonds which are required to form the joined-up
        constructed molecule, but note that it does not have to add any
        bonds at all. In addition, a :class:`.Reaction` can add and
        remove atoms from the constructed molecule. Which reaction is
        selected to join the functional groups depends on the
        :class:`.ReactionFactory` given to the :class:`.TopologyGraph`
    ...
    def __init__(
        ...
        construction_stages:
            tuple[typing.Callable[[Vertex], bool], ...],
        num_processes: int,
        optimizer: Optimizer,
        ...
        Parameters:
            ...
            construction_stages:
                ...
                into serial stages if synchronization between stages is
                needed.

                If the topology graph is performing construction
                serially, then all vertices which belong to an earlier
                stage will place their building block before those at a
                later stage.
        ...
        self._building_block_vertices = {
            building_block: tuple(map(
                lambda vertex: vertex.with_scale(scale),
                vertices,
            ))
            for building_block, vertices
            in building_block_vertices.items()
        }
        self._edges = tuple(map(
            lambda edge: edge.with_scale(scale),
            edges,
        ))
    ...
    def _with_building_blocks(
        ...
        def get_new_edge(edge_id: int) -> Edge:
