...
class RandomSmarts(MoleculeMutator):
    ...
    def __init__(
        ...
        replacement_fgs : :class:`list` of
         :class:`.FunctionalGroupFactory`
            The functional group factories used to assign functional
            groups within the generated molecule.
        ...
        self._replacement_fgs = replacement_fgs
    ...
    def mutate(self, record):
        ...
        if self._replacement_specifier == 'all':
            ...
            raise RuntimeError(
                'Invalid argument for replacement_specifier received'
            )
        # Create new BuildingBlock
        replacement = BuildingBlock.init_from_rdkit_mol(
            new_rdmol, functional_groups=self._replacement_fgs
        )
