...
class GAProgress:
    ...
    def normalize(self, norm):
        ...
        Applies the normalization functions in `norm` to the Population
        in the `pops` attribute. It then uses the new fitness values to
        update the values in `means`, `mins` and `maxs`.


        Parameters
        ----------
        norm : Normalization
            A Normalization object which carries out the normalization.
